...
void CudaKernels::consolidate() {
   ...
   // cudaStreams = = (cudaStream_t *)malloc(sizeof(cudaStream_t) * ngpus);
    ...
    cudaHostAlloc(&hostInput, sizeof(CudaInputElem)*sizeof(int), cudaHostAllocDefault);//zero-copy
    ...
    constInput = (CudaInputElem **)malloc(sizeof(CudaInputElem * )*ngpus); //for persistant kernel
    //constInput = ()
...
void CudaKernels::processMessage(CudaOutbox *outbox,int dev_no) {
  ...
  int maxConstr = copyMsgToDevice(outbox->message,dev_no);//maxConstr = 10211
  ...
  if (maxConstr>0) {
    ...
    for(int i=0; i<numValues; i++) {
	  evalConstraint<<<hostInput[dev_no][i].numConstraints/NUM_THREADS+1, NUM_THREADS>>>(currentFiltersCount[dev_no], filtersInfo[dev_no], interfacesDevice[dev_no], numFilters, numInterfaces, i,hostInput[dev_no]);
	}
	//cudaStreamSynchronize((cudaStream_t)stream1);
...
void CudaKernels::computeResults(int maxConstr,int dev_no) {
	...
	//dim3 numBlocks = dim3(numBlocksX);
      /*udaStream_t *streamk = (cudaStream_t *)malloc(sizeof(cudaStream_t) * dev_no); 
       cudaStream_t *streamc = (cudaStream_t *)malloc(sizeof(cudaStream_t) * dev_no) ;
        for(int i = 0 ; i  < dev_no; i++)
       {  
             cudaStreamCreate(&streamk[i]);
